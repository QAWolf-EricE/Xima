...
test("after_hours_ivr", async () => {
 // Step 1. After Hours IVR
  //--------------------------------
  // Arrange:
  //--------------------------------
  ...
  let params = {};
  const baseUrl = "https://xima-after-hours-ivr-5939.twil.io";
  ...
  // Check to make sure the correct call was opened
  const targetLocator = page.locator(
    `[data-cy="cradle-to-grave-table-cell-event-name"] :text("Time of Day Check - Out of Hours IVR")`,
  );
  ...
  try {
    ...
    tries = 0;
    let foundRow = false;
    while (!targetCallTime && tries < 10 && !foundRow) {
      tries++;
  ...
  /* 
    Assert there is a row that contains "Auto Attendant" and "Time of Day Check - Out of Hours"
    This means the caller sucessfully transferred to the "Out of Hours" menu
  */
  await expect(
    page.getByRole(`cell`, {
      name: `Time of Day Check - Out of Hours IVR`,
      exact: true,
    }),
  ).toBeVisible();
  
  /* 
    Assert there is a row that contains "Queue Audio" and "Out of Hours"
    This means the caller was sucessfully transferred to the Out of Hours Queue
  */
  await expect(
    page
