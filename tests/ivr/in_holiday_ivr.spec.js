...
test("in_holiday_ivr", async () => {
 // Step 1. In Holiday IVR
  //--------------------------------
  // Arrange:
  //--------------------------------
  ...
  let params = {};
  const startUrl = `https://xima-ivr-9663.twil.io/start-call?uniqueIdentifier=${uniqueIdentifier}`;
  const checkUrl = `https://xima-ivr-9663.twil.io/check-results?uniqueIdentifier=${uniqueIdentifier}`;
  ...
  // expand target call row if needed
  await expect(page.locator(
      `[data-cy="cradle-to-grave-table-row"]:has(:text("${targetCallTime}"))`,
    )).toBeVisible();
  try {
    await page
      .locator(
      ...
      .click({ timeout: 5000 });
  } catch { console.log("Row is already expanded.") }
  
  // Check to make sure correct call was opened
  const targetLocator = page.locator(
    `[data-cy="cradle-to-grave-table-cell-event-name"] :text("Announcement - In Holiday Announcement")`,
  );
  ...
  // Assert:
  //--------------------------------
  // Assert there are 2 (two) row that contains "Auto Attendant" and "Test Holidays"(
  ...
  await expect(
    page.locator(
      `[data-cy="cradle-to-grave-table-row-details-row"]:has-text("Auto Attendant"):has-text("Test Holidays"):has-text("${dateFns.format(currDate, "MM/dd/yyyy")}")`,
    ).first()
  ).toBeVisible();
  
  // Assert there is a row that contains "Queue Audio" and "In Holiday"(
  // This means the caller was sucessfully tranfered to the In Holiday Queue
  //)
  await expect(
    page.locator(
      `[data-cy="cradle-to-grave-table-row-details-row"]:has-text("Queue Audio"):has-text("In Holiday"):has-text("${dateFns.format(currDate, "MM/dd/yyyy")}"):visible >> nth=0`,
    ),
  ).toBeVisible();
