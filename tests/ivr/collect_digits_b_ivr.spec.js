...
test("collect_digits_b_ivr", async () => {
 // Step 1. Collect Digits B IVR
  //--------------------------------
  // Arrange:
  //--------------------------------
  // Consts
  ...
  const params = { menu1digit: "3", menu2digit: "2" };
  const auth =
    "Basic " + Buffer.from(`${user}:${userPassword}`).toString("base64");
  const queryParams = `&menu1digit=3&menu2digit=2`;
  const skill = "Collect Digits B";
  
  // Login as WebRTC agent
  const { browser, context, page } = await logInWebRTCAgent(
    process.env.WEBRTCAGENT_52_EMAIL,
    {
      args: [
  ...
  // Assert there is a row that contains "Auto Attendant" and "Collect Digits B Hide Agent"(
  // This means the caller sucessfully tranfered to "Collect Digits B"
  // )
  await expect(
    adminPage.locator(
      `[data-cy="cradle-to-grave-table-row-details-row"]:has-text("Auto Attendant"):has-text("Collect Digits B Hide Agent"):has-text("${mountainDate}")`,
    ),
  ).toBeVisible();
  ...
  await adminPage
    .getByRole(`menuitem`, { name: `View Session Parameters` })
    .click();
  
  // Assert the collected digits are viewable in C2G
  await expect(
    adminPage.locator(
      `.session-parameter:text("AccountNum: ${process.env.COLLECT_DIGITS_ACC}")`,
    ),
  ).toBeVisible();
