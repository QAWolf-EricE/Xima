...
test("collect_digits_a_ivr", async () => {
 // Step 1. Collect Digits A IVR
  // Please see: https://docs.google.com/spreadsheets/d/1lQpQLW626ildlPUukqZNPc4CcZQjrKSEr1_NaiI5kEo/edit?usp=sharing
  // See "Collect Digits A IVR"
  // Functions are on Twilio
  //--------------------------------
  // Arrange:
  //--------------------------------
  // Consts
  ...
  await page.click(`span:text("${skill}") + mat-slide-toggle`);
  await page.waitForTimeout(1000);
  ...
  console.log(mountainTimeZone);
  const easternTimeZone = date.toLocaleString("en-US", {
    timeZone: "America/New_York",
    month: "numeric",
    day: "2-digit",
    year: "numeric",
  });
  console.log(easternTimeZone);
  ...
  try { // expand target call row
    await adminPage
      .locator(
        `[data-cy="cradle-to-grave-table-row"]:has(:text("${targetCallTime}")) [data-cy="cradle-to-grave-table-expand-row-button"]`,
      )
    .click({ timeout: 3000 });
  } catch {  // Refresh page and try again
    await adminPage.locator(`[data-cy="cradle-to-grave-toolbar-refresh-button"]`).click();
    await adminPage
      .locator(
        `[data-cy="cradle-to-grave-table-row"]:has(:text("${targetCallTime}")) [data-cy="cradle-to-grave-table-expand-row-button"]`,
      )
  ...
  // Assert:
  //--------------------------------
  // Assert agent can view collected digits
  await page.bringToFront();
  await adminPage.bringToFront()
  // Assert Session Parameters are visible
  const sessionParams = page.locator(`.data:has-text("Session Parameters")`);
  await expect(sessionParams).toBeVisible();
  
  // Assert agent can view collected digits
  await expect(page.locator(`.session-parameter:text("Account: ${process.env.COLLECT_DIGITS_ACC}")`)).toBeVisible();
  
  // Assert call routed to Data Dip Skill
  await expect(page.locator(`[data-cy="details-sidebar-details-MEDIA_TARGET_SKILL"]`)).toHaveText(skill);
  
  // Assert Data dip succeeded
  await expect(page.locator(`.session-parameter:text("Data Dip: SUCCESS")`)).toBeVisible();
  ...
  await expect(
    adminPage.locator(
      `[data-cy="cradle-to-grave-table-row-details-row"]:has-text("Auto Attendant"):has-text("CD and DD Menu"):has-text("${easternTimeZone}")`,
    ),
  ).toBeVisible();
  
  // Assert there is a row that contains "Auto Attendant" and "Collect Digits A w Data"(
  // This means the caller sucessfully tranfered to "Collect Digits A"
  // )
  await expect(
    adminPage.locator(
      `[data-cy="cradle-to-grave-table-row-details-row"]:has-text("Auto Attendant"):has-text("Collect Digits A w Data"):has-text("${easternTimeZone}")`,
    ),
  ).toBeVisible();
  
  // Assert there is a row that contains "Queue Audio" and "Data Dip Skill"(
  // This means the caller was sucessfully tranfered to the Data Dip Skill Queue
  //)
  await expect(
    adminPage.locator(
      `[data-cy="cradle-to-grave-table-row-details-row"]:has-text("Queue Audio"):has-text("${skill}"):has-text("${easternTimeZone}"):visible >> nth=0`,
    ),
  ).toBeVisible();
  ...
  // Click "View session parameters"
  await adminPage.getByRole(`menuitem`, { name: `View Session Parameters` }).click();
  
  // Assert the collected digits are viewable in C2G
  await expect(adminPage.locator(`.session-parameter:text("Account: ${process.env.COLLECT_DIGITS_ACC}")`)).toBeVisible();
  
  // Assert Data dip succeeded
  await expect(adminPage.locator(`.session-parameter:text("Data Dip: SUCCESS")`)).toBeVisible();
