...
test("in_hours_ivr", async () => {
 // Step 1. IN Hours IVR
  ...
  let params = {};
  const baseUrl = "https://xima-in-hours-ivr-5651.twil.io";
  ...
  const targetLocator = page.locator(
    `[data-cy="cradle-to-grave-table-cell-event-name"] :text("Time of Day Check - In Hours")`,
  );
  ...
  /* 
    Assert there is a row that contains "Auto Attendant" and "Time of Day Check - In Hours"
    This means the caller sucessfully transferred to the "In Hours" menu
  */
  await expect(
    page.getByRole(`cell`, { name: `Time of Day Check - In Hours`, exact: true }),
  ).toBeVisible();
  
  /* 
    Assert there is a row that contains "Queue Audio" and "In Hours"
    This means the caller was sucessfully transferred to the In Hours Queue
  */
  await expect(
    page
