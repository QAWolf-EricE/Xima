...
test("standard_param_ivr", async () => {
 // Step 1. Stand Param IVR
  //--------------------------------
  // Arrange:
  //--------------------------------
  ...
  const params = { menu1digit: "5", menu2digit: "1" };
  const queryParams = `&menu1digit=5&menu2digit=1`;
  ...
  const skill = "Standard Parameter Condition Skill";
  ...
  // filter by Calls made today to the "Standard Parameter Condition Skill" Skill
  await setFilter({
    page,
    channel: "Calls",
    skill,
  ...
  const targetLocator = page.locator(
    `[data-cy="cradle-to-grave-table-cell-event-name"] :text("Parameter Check - Check DNIS")`,
  );
  ...
  // Assert:
  //--------------------------------
  // Assert there is a row that contains "Auto Attendant" and "Parameter Check - Check DNIS"(
  // This means the caller sucessfully tranfered to the "Standard Parameter" menu
  // )
  await expect(
    page
      .getByRole(`cell`, { name: `Parameter Check - Check DNIS`, exact: true })
