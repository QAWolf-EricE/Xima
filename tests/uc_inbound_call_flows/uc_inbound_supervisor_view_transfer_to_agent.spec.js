import { createCall, inputDigits, logInAgent, logInSupervisor, logUCAgentIntoUCWebphone, toggleStatusOn } from '../../lib/node_20_helpers';
import { assert, axios, crypto, dateFns, dotenv, expect, faker, formatInTimeZone, fse, getInbox, https, launch, saveTrace, test, twilio } from '../../qawHelpers';


test("uc_inbound_supervisor_view_transfer_to_agent", async () => {
 // Step 1. Supervisor can transfer a UC call that is queued on a skill to an agent that does not have that skill ready
  //--------------------------------
  // Arrange:
  //--------------------------------
  // Test 1: Call comes into the system and queues a skill. 
  // An Agent (phone user) is logged in but is not ready in the skill. 
  ...
  // Declare constants
  const agentName = 'Xima Agent 15(115)'
  const skillNumber = 30
  
  // Login to a UC Agent stephanie (do not Ready this agent)
  const { page: agentPage, browser: agentBrowser, context: agentContext } = await logInAgent({
    email: process.env.UC_AGENT_15_EXT_115,
    password: process.env.UC_AGENT_15_EXT_115_PASSWORD,
    args: [
  ...
  const { ucWebPhonePage } = await logUCAgentIntoUCWebphone(agentContext, process.env.UC_AGENT_15_EXT_115_WEBPHONE_USERNAME);
  try {
    await ucWebPhonePage.locator(`:text("NEXT")`).click();
    await ucWebPhonePage.locator(`:text("done")`).click();
  ...
  // Login to Supervisor (admin)
  await agentPage.waitForTimeout(1000)
  const { page: supervisorPage, browser: supervisorBrowser, context: supervisorContext } = await logInSupervisor({ timezoneId: "America/Denver" })
  ...
  await inputDigits(callId, [`0`]); // press 0 for skill 30
  ...
  await supervisorPage.locator(`mat-option:has-text("Skill ${skillNumber}")`).click();
  
  await supervisorPage.locator(`.queued-calls-content-row-name>>span`).waitFor();
  ...
  // Expect the element with id "accordion-body-0" that has the text "Skill8/Xima Live Media" to be visible
  await expect(
  ...
  // Assert skill number is correct and visible
  await expect(agentPage.locator(`:text("Skill ${skillNumber}")>>nth=0`)).toBeVisible();
