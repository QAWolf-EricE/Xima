...
test.describe('Report Correctness - External Number Summary', () => {
  test('External Number Summary report displays correct external number call data', async ({ page }) => {
    const reportName = 'External Number Summary';
    ...
    const result = await reportCorrectnessPage.executeReportCorrectnessWorkflow({
      ...
      requiredFields: ['External Number', 'Call Count', 'Total Duration'],
      summaryFields: ['Total External Numbers', 'Total Calls'],
      validations: [{ field: 'Total External Numbers', type: 'number' }, { field: 'Total Calls', type: 'number' }]
    });
