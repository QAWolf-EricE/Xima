...
test.describe('Report Correctness - Inbound Calls by Local Number', () => {
  test('Inbound Calls by Local Number report displays correct local number call data', async ({ page }) => {
    ...
    const result = await reportCorrectnessPage.executeReportCorrectnessWorkflow({
      ...
      summaryFields: ['Total Local Numbers', 'Total Inbound Calls'],
      validations: [{ field: 'Total Local Numbers', type: 'number' }, { field: 'Total Inbound Calls', type: 'number' }]
    });
