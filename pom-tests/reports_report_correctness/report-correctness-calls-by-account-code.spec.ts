...
test.describe('Report Correctness - Calls by Account Code', () => {
  test('Calls by Account Code report displays correct account code call data', async ({ page }) => {
    const reportName = 'Calls By Account Code';
    ...
    const result = await reportCorrectnessPage.executeReportCorrectnessWorkflow({
      ...
      requiredFields: ['Account Code', 'Call Count', 'Total Duration'],
      summaryFields: ['Total Account Codes', 'Total Calls'],
      validations: [{ field: 'Total Account Codes', type: 'number' }, { field: 'Total Calls', type: 'number' }]
    });
